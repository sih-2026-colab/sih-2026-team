...
for t=0:opts.dt:opts.duration
    ...
    if opts.visualize
        if isempty(scene), scene=autonex_animation_scene('create',out.actors,-1.75,8.75,opts.showLaneMarkings); end
        scene=autonex_animation_scene('update',scene,out.actors,out.candidates,out.selected,out.tracks,out);
        drawnow;
        if opts.exportVideo, writeVideo(video,autonex_video_frame(scene.figure)); end
    end
...
% Legacy samples are pre-step; expose the actual final integrated state too.
report.finalEgo=state.ego;
report.integratedThroughTime=samples(end).time+opts.dt;
